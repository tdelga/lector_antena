...
public class ZLDMHandler {
	 ...
	 */
	public boolean dllInit() {
		String dllName = "dll/ZLDevManage";
		boolean inital = false;
		try{
			inital = ZLDMOperate.init(dllName);
			selectedDevNo =  (byte) startSearchDev();
			selectedDevNo = (byte) (selectedDevNo - 1);
		}catch(Exception e){
			System.out.println("调用 dll/ZLDevManage  文件失败!");
		}
		return inital;
	}
